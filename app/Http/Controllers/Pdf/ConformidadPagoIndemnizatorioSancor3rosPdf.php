...
class ConformidadPagoIndemnizatorioSancor3rosPdf extends fpdf {
	...
	function body() {
		$this->y +=3;
		$text = $this->siniestro->asegurado.', DNI '.$this->siniestro->numero_documento.', por medio de la presente, en mi carácter de asegurado, en virtud del contrato de seguro Hogar Individual, individualizado bajo la póliza N° '.$this->siniestro->poliza->numero_poliza.', referencia N° '.$this->siniestro->poliza->referencia.', ACEPTO percibir el monto indemnizatorio determinado por Sancor Cooperativa de Seguros Limitada, fijado en la suma de '.$this->siniestro->liquidacion_siniestro.', en concepto de único pago indemnizatorio, total y definitivo, cancelatorio de todas las obligaciones emergentes del contrato de seguro antes individualizado, en relación con el siniestro denunciado el día '.date_format($this->siniestro->fecha_denuncia, 'd/m/Y').' como ocurrido el día '.date_format($this->siniestro->fecha_ocurrencia, 'd/m/Y').', producto de otras causas, ocurrido en '.$this->siniestro->domicilio_completo_google.'.';
		$this->x = 15;
