...
class DesgasteYOViciosPropiosPdf extends fpdf {
	...
	function __construct($siniestro) {
		...
		$this->AddPage();
        $this->Output('I', str_replace('#', '-', $this->siniestro->numero_siniestro).' Desgaste y-o vicios propios.pdf');
        exit;
	}
	...
	function printData() {
		...
		$this->y += 5;
		$this->x = 15;
		$text = 'Al respecto, debemos comunicarle que esta Aseguradora procede mediante el presente al rechazo del referido siniestro, en virtud de tratarse de un HECHO NO CUBIERTO por la Póliza de Seguro de Hogar Ref. '.$this->siniestro->referencia.', oportunamente contratado, que excluye expresamente los daños provocados por el uso o desgaste normal, vicio propio o defectuosa de los bienes asegurados.';
